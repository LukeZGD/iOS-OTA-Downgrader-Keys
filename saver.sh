...
# Formerly used as manifest and keys saver
# Will now be used as a script to update URLs in iOS-OTA-Downgrader-Keys in the event that they change in Apple and ipsw.me

for ProductType in "${devices[@]}"
do
    mkdir -p $ProductType/12H321
    ...
    #curl -L https://firmware-keys.ipsw.me/firmware/$ProductType/12H321 -o $ProductType/12H321/index.html
    echo "$ProductType sha1sum and url"
    curl -L https://api.ipsw.me/v2.1/${ProductType}/12H321/sha1sum -o $ProductType/12H321/sha1sum
    curl -L https://api.ipsw.me/v2.1/${ProductType}/12H321/url -o $ProductType/12H321/url
    #HWModelSet
    #IV=IV_$HWModel
    #Key=Key_$HWModel
    #echo $Key | tee $ProductType/12H321/key
    #echo $IV | tee $ProductType/12H321/iv
done

for ProductType in "${devices613[@]}"
do
    mkdir -p $ProductType/10B329
    ...
    #curl -L https://firmware-keys.ipsw.me/firmware/$ProductType/10B329 -o $ProductType/10B329/index.html
    curl -L https://api.ipsw.me/v2.1/${ProductType}/10B329/sha1sum -o $ProductType/10B329/sha1sum
    curl -L https://api.ipsw.me/v2.1/${ProductType}/10B329/url -o $ProductType/10B329/url
    #HWModelSet
    #IV=IV_${HWModel}_613
    #Key=Key_${HWModel}_613
    #echo $Key | tee $ProductType/10B329/key
    #echo $IV | tee $ProductType/10B329/iv
done

mkdir -p iPad2,2/13G36
curl -L https://api.ipsw.me/v2.1/iPad2,2/13G36/url -o iPad2,2/13G36/url

for ProductType in iPad2,3 iPad2,6 iPad2,7 iPad3,2 iPad3,3 iPhone4,1
do
    mkdir -p $ProductType/13G37
    curl -L https://api.ipsw.me/v2.1/${ProductType}/13G37/url -o $ProductType/13G37/url
done

for ProductType in iPhone5,1 iPhone5,2 iPad3,5 iPad3,6
do
    mkdir -p $ProductType/14G61
    curl -L https://api.ipsw.me/v2.1/${ProductType}/14G61/url -o $ProductType/14G61/url
done
